...
public abstract class PageView extends ViewGroup {
	...
	protected     ArrayList<pdfShape> mDrawing;
			...
			mSearchView = new View(mContext) {
				...
				protected void onDraw(final Canvas canvas) {
					...
					if (mDrawing != null) {
						...
						Path path = new Path();	
						Iterator<pdfShape> it = mDrawing.iterator();
						while (it.hasNext()) {
							pdfShape arc = it.next();
							if (arc.size() >= 2||arc.type==Annotation.Type.TEXT) {
								if(arc.type!=Annotation.Type.TEXT){
									Iterator<PointF> iit = arc.getPoints().iterator();	
									p = iit.next();
									float mX = p.x * scale;
									float mY = p.y * scale;
									...
									while (iit.hasNext()) {
										...
										float x = p.x * scale;
										float y = p.y * scale;
										if(arc.type==Annotation.Type.INK) path.quadTo(mX, mY, (x + mX) / 2, (y + mY) / 2);
									...
									//path.addRect((arc.getX())*scale, (arc.getY())*scale,(arc.getX()+100)*scale,(arc.getY()-100)*scale,Direction.CW);
									path.addCircle((arc.getX()+10)*scale, arc.getY()*scale,50*scale,Direction.CW);
									paint.setColor(arc.getColor());
									paint.setStyle(Paint.Style.FILL);
									canvas.drawPath(path, paint);
									path.reset();
	...
	public void startDraw(float x, float y, int shape, int color) {
		float scale = mSourceScale*(float)getWidth()/(float)mSize.x;
		float docRelX = (x - getLeft())/scale;
		float docRelY = (y - getTop())/scale;
		if (mDrawing == null)
			mDrawing = new ArrayList<pdfShape>();

		/*pdfShape arc = new pdfShape(docRelX, docRelY);
		arc.add(new PointF(docRelX, docRelY));*/
			switch (shape) {
			case 0:
				mDrawing.add(new pdfFree(docRelX, docRelY,color));
				break;
			case 1:
				mDrawing.add(new pdfRect(docRelX, docRelY,color));
				break;
			case 2:
				mDrawing.add(new pdfArrow(docRelX, docRelY,color));
				break;
			case 3:
				mDrawing.add(new pdfLine(docRelX, docRelY,color));
				break;
	...
	public void continueDraw(float x, float y) {
		...
		if (mDrawing != null && mDrawing.size() > 0) {
			pdfShape arc = mDrawing.get(mDrawing.size() - 1);
			arc.setEnd(docRelX, docRelY);
			mSearchView.invalidate();
		}
	}
	...
	public void AddTextAnnotation(float x, float y, String text) {
		float scale = mSourceScale*(float)getWidth()/(float)mSize.x;
		float docRelX = (x - getLeft()-50)/scale;
		float docRelY = (y - getTop()+5)/scale;
		if (mDrawing == null)
			mDrawing = new ArrayList<pdfShape>();

		mDrawing.add(new pdfNote(docRelX, docRelY,text,scale));
		mSearchView.invalidate();
	}

	protected pdfShape[] getDraw() {
		if (mDrawing == null)
			return null;

		pdfShape[] path = mDrawing.toArray(new pdfShape[mDrawing.size()]);

		/*for (int i = 0; i < mDrawing.size(); i++) {
			pdfShape arc = mDrawing.get(i);
			path[i] = arc.getPoints().toArray(new PointF[arc.size()]);
		}*/
